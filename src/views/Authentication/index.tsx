...
//                    interface                    //
interface Props {
  onLinkClickHandler: () => void // 함수를 받아올 때 타입 지정
}
...
function SignUp ({onLinkClickHandler} : Props) {
  ...
  const [isPasswordPattern, serPasswordPattern] = useState<boolean>(false);
  ...
  const onPasswordChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    serPasswordPattern(isPasswordPattern);

    const passwordMessage = isPasswordPattern ? '' : value ? '영문, 숫자를 혼용하여 8 ~ 13자리를 입력해주세요.' : '';
  ...
  const onSignUpButtonClickHandler = () => {
    ...
    if (!id || !password || !email || !authNumber) {
      alert('모든 내용을 입력해주세요.');
      return;
    };
    ...
    <div className="authentication-contents">
      ...
      <div className="authentication-input-container">
        ...
        <InputBox label="아이디" type="text" value={id} placeholder="아이디를 입력해주세요" onChangeHandler = {onIdChangeHandler} buttonTitle="중복 확인" buttonStatus={idButtonStatus}
          onButtonClickHandler={onIdButtonClickHandler} message={idMessage} error = {isIdError} />
        ...
        <InputBox label="이메일" type="text" value={email} placeholder="이메일를 입력해주세요" onChangeHandler = {onEmailChangeHandler} buttonTitle="이메일 인증" buttonStatus={emailButtonStatus}
          onButtonClickHandler={onEmailButtonClickHandler} message={emailMessage} error = {isEmailError}/>
        
        { isEmailCheck &&  
        <InputBox label="인증번호" type="text" value={authNumber} placeholder="인증번호 4자리를 입력해주세요" onChangeHandler = {onAuthNumberChangeHandler} buttonTitle="인증 확인" buttonStatus={authNumberButtonStatus}
